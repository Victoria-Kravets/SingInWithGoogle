...
class ApiLayer {
    ...
    static let SharedApiLayer = ApiLayer(
        urlScheme: ApiLayerConstants.ProvengURLScheme,
        urlHost: ApiLayerConstants.ProvengURLHost,
        urlPath: ApiLayerConstants.ProvengURLPath)
        ...
        return firstly {
            self.requestForResult(apiMethod)
                ...
                guard let object = data["result"] as? [String: AnyObject] else {
                    return Promise(error: ApiError(code: 048,
                                                   userInfo:
                        [NSLocalizedDescriptionKey as NSObject: "No object in result"
                            as AnyObject]))
            ...
            sendRawRequest(apiMethod) { result in
                ...
                case .success(let value):
                    ...
                    guard data[ApiLayerConstants.ResultKey] != nil else {
                        ...
                        if let errorDict = data[ApiLayerConstants.ErrorKey],
                            let errorDescription = errorDict[ApiLayerConstants.ErrorDescriptionKey] as? String,
                            let errorType = errorDict[ApiLayerConstants.ErrorTypeKey] as? String {
                            var intErrorCode = 0
                            if let errorCode = errorDict[ApiLayerConstants.ErrorCodeKey] as? String,
                                let intError = Int(errorCode) {
                                intErrorCode = intError
                            } else if let errorCode = errorDict[ApiLayerConstants.ErrorCodeKey] as? Int {
                                intErrorCode = errorCode
                            }
                            apiError = ApiError(domain: errorType,
                                                code: intErrorCode,
    ...
    func sendRawRequest(_ apiMethod: ApiMethod, completion: @escaping (Alamofire.Result<Any>) -> Void) {
        ...
        let request = manager.request(urlRequest!,
                                      ...
                                      encoding: body, headers: additionalHeaders)
            .responseJSON { [weak self] response in
                ...
                case .success:
                    ...
                    print("JSON From Backend: \(response.result.value ?? 0)")
                    guard response.result.value != nil else {
                        let error: ApiError = ApiError(domain: self?.urlHost,
                                                       userInfo:
